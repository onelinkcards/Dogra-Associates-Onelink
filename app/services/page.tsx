...
function ServicesPageInner() {
  ...
  const handleSendGeneralInquiry = () => {
      ...
      >
        <div className="flex flex-col gap-2">
          {selectedCount === 0 && (
            <button
              type="button"
              onClick={handleSendGeneralInquiry}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 rounded-full font-semibold text-[15px] text-white py-3.5 px-5 shadow-lg transition-all duration-200 touch-manipulation disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] disabled:active:scale-100"
            style={{
              background:
                selectedCount > 0
                  ? `linear-gradient(135deg, ${ACCENT} 0%, ${ACCENT_LIGHT} 100%)`
          ...
          >
            <span
              ...
              style={{
                background: selectedCount > 0 ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.2)',
              }}
            >
              {selectedCount}
            </span>
