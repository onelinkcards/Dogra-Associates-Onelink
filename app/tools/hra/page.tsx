'use client'

import { useState } from 'react'
import { motion } from 'framer-motion'
import ToolLayout from '../components/ToolLayout'
import { ToolInput, toolLabelClass } from '../components/ToolInput'
import ConversionCTA from '../components/ConversionCTA'
import { shopConfig } from '../../shops/honeys-fresh-n-frozen/config'
import { getWhatsAppLink } from '../../lib/phone'

// HRA exemption: min(actual HRA, 50% or 40% of basic for metro/non-metro, rent - 10% of basic)
function calcHRAExemption(
  basic: number,
  hraReceived: number,
  rentPaid: number,
  isMetro: boolean
): number {
  if (basic <= 0 || hraReceived <= 0) return 0
  const percentOfBasic = isMetro ? 0.5 : 0.4
  const component1 = hraReceived
  const component2 = percentOfBasic * basic
  const component3 = Math.max(0, rentPaid - 0.1 * basic)
  const exemption = Math.min(component1, component2, component3)
  return Math.round(exemption)
}

export default function HRACalculatorPage() {
  const [basic, setBasic] = useState('')
  const [hraReceived, setHraReceived] = useState('')
  const [rentPaid, setRentPaid] = useState('')
  const [cityType, setCityType] = useState<'metro' | 'nonmetro'>('metro')
  const [hasCalculated, setHasCalculated] = useState(false)

  const basicNum = Math.max(0, parseInt(basic, 10) || 0)
  const hraNum = Math.max(0, parseInt(hraReceived, 10) || 0)
  const rentNum = Math.max(0, parseInt(rentPaid, 10) || 0)
  const isMetro = cityType === 'metro'

  const exemption = calcHRAExemption(basicNum, hraNum, rentNum, isMetro)
  const taxableHRA = Math.max(0, hraNum - exemption)
  const showResults = hasCalculated && basicNum > 0 && hraNum > 0

  return (
    <>
      <ToolLayout
        title="HRA Calculator"
        subtitle="Exemption under House Rent Allowance"
      >
        <div className="space-y-4">
          <div>
            <label className={toolLabelClass}>Basic Salary</label>
            <div className="flex items-center gap-2">
              <span className="text-[15px] text-[#6b7280]">₹</span>
              <ToolInput
                type="number"
                value={basic}
                onChange={(e) => setBasic(e.target.value)}
                placeholder="e.g. 50000"
              />
            </div>
          </div>

          <div>
            <label className={toolLabelClass}>HRA Received</label>
            <div className="flex items-center gap-2">
              <span className="text-[15px] text-[#6b7280]">₹</span>
              <ToolInput
                type="number"
                value={hraReceived}
                onChange={(e) => setHraReceived(e.target.value)}
                placeholder="e.g. 20000"
              />
            </div>
          </div>

          <div>
            <label className={toolLabelClass}>Rent Paid</label>
            <div className="flex items-center gap-2">
              <span className="text-[15px] text-[#6b7280]">₹</span>
              <ToolInput
                type="number"
                value={rentPaid}
                onChange={(e) => setRentPaid(e.target.value)}
                placeholder="e.g. 25000"
              />
            </div>
          </div>

          <div>
            <label className={toolLabelClass}>City Type</label>
            <div className="flex flex-col gap-2 pt-1">
              <label className="flex items-center gap-2.5 cursor-pointer text-[15px] text-[#374151]">
                <input
                  type="radio"
                  name="cityType"
                  checked={cityType === 'metro'}
                  onChange={() => setCityType('metro')}
                  className="w-4 h-4 accent-[#3B82F6]"
                />
                Metro
              </label>
              <label className="flex items-center gap-2.5 cursor-pointer text-[15px] text-[#374151]">
                <input
                  type="radio"
                  name="cityType"
                  checked={cityType === 'nonmetro'}
                  onChange={() => setCityType('nonmetro')}
                  className="w-4 h-4 accent-[#3B82F6]"
                />
                Non-Metro
              </label>
            </div>
          </div>
        </div>

        <button
          type="button"
          onClick={() => setHasCalculated(true)}
          className="w-full mt-6 py-[14px] px-4 rounded-[12px] font-semibold text-[15px] text-white transition-opacity hover:opacity-95"
          style={{ background: '#334155' }}
        >
          Calculate
        </button>

        {showResults && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mt-6 rounded-2xl border border-[#e5e7eb] p-5 bg-white"
            style={{ boxShadow: '0 6px 14px rgba(0,0,0,0.06)' }}
          >
            <h3 className="text-[16px] font-semibold text-[#111827] mb-4">
              Result
            </h3>
            <div className="space-y-3">
              <div className="flex justify-between text-[14px]">
                <span className="text-[#6b7280]">HRA Exemption</span>
                <span className="font-semibold text-[#111827]">
                  ₹{exemption.toLocaleString('en-IN')}
                </span>
              </div>
              <div className="flex justify-between text-[14px] pt-2 border-t border-[#e5e7eb]">
                <span className="text-[#6b7280]">Taxable HRA</span>
                <span className="font-semibold text-[#111827]">
                  ₹{taxableHRA.toLocaleString('en-IN')}
                </span>
              </div>
            </div>
            <ConversionCTA />
          </motion.div>
        )}
      </ToolLayout>

      {showResults && (
        <div
          className="fixed bottom-0 left-0 right-0 z-30 py-3 px-4 safe-area-pb flex gap-2.5"
          style={{
            background: 'white',
            boxShadow: '0 -4px 20px rgba(0,0,0,0.08)',
            maxWidth: 420,
            margin: '0 auto',
          }}
        >
          <button
            type="button"
            onClick={() => {
              sessionStorage.setItem('openAppointment', 'true')
              window.location.href = '/'
            }}
            className="flex-1 py-3 px-4 rounded-[12px] font-semibold text-[15px] text-white transition-opacity hover:opacity-95"
            style={{ background: '#334155' }}
          >
            Book Consultation
          </button>
          <a
            href={getWhatsAppLink(shopConfig.contact.clientPhoneE164 || '919086038829', "Hello Sir, I used the HRA Calculator. I'd like to optimise my tax.")}
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 py-3 px-4 rounded-[12px] font-semibold text-[15px] text-center border transition-opacity hover:opacity-90"
            style={{ borderColor: '#334155', color: '#334155', background: 'white' }}
          >
            Ask on WhatsApp
          </a>
        </div>
      )}
    </>
  )
}
